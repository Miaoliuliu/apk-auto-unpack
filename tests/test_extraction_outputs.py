...
- build_source_index 按原文索引，HTTP/HTTPS 保持独立
...
def test_source_index_keeps_http_https_variants_separate():
    ...
    assert _tags(idx["http://a.cn/api"]) == ["dex/string_pool@c1.dex"]
    assert _tags(idx["https://a.cn/api"]) == ["assets/harvest@cfg.json"]
...
def test_write_url_files_annotates_source(tmp_path: Path):
    ...
    assert _BIZ in body
    assert _NOISE not in body
    assert _BARE not in body
    assert "未写入" in text
...
def test_write_url_files_non_comment_lines_are_urls(tmp_path: Path):
    ...
    assert urls
    for ln in urls:
        assert ln == ln.split()[0]
        assert ln.startswith(("http://", "https://", "ws://", "wss://"))
...
def test_write_url_files_header_reports_counts_and_legend(tmp_path: Path):
    ...
    assert "未写入" in head_text
    assert "端点路径 2" in head_text
    assert "来源为注释" in head_text
...
def test_write_url_files_writes_machine_readable_outputs(tmp_path: Path):
    (tmp_path / "urls.txt").write_text("old", encoding="utf-8")
    write_url_files(tmp_path, {_BIZ}, set())
    assert (tmp_path / "urls.txt").read_text(encoding="utf-8") == _BIZ + "\n"
    jsonl = (tmp_path / "indicators.jsonl").read_text(encoding="utf-8")
    assert '"validation"' in jsonl
    assert _BIZ in jsonl


def test_write_url_files_rejects_invalid_legacy_input(tmp_path: Path):
    bad = "https://bad host.example/api"
    write_url_files(tmp_path, {_BIZ, bad}, set(), [
        {"url": _BIZ, "sources": []},
        {"url": bad, "sources": []},
    ])
    ranked = (tmp_path / "urls_by_rank.txt").read_text(encoding="utf-8")
    absolute = (tmp_path / "urls.txt").read_text(encoding="utf-8")
    jsonl = (tmp_path / "indicators.jsonl").read_text(encoding="utf-8")
    assert _BIZ in ranked and _BIZ in absolute and _BIZ in jsonl
    assert bad not in ranked
    assert bad not in absolute
    assert bad not in jsonl
...
def test_skip_junk_list_assets_and_binary():
    from auto_unpack.extraction.analyze import (
        _should_binary_scan_entry,
        _skip_asset,
    )
    assert _skip_asset("assets/keyword.txt") is True
    assert _skip_asset("assets/bad_domains.txt") is True
