...
def _is_ip_host(host: str) -> bool:
    return bool(host and _IP_HOST.match(host))
...
def _netloc(scheme: str, host: str, port: int | None) -> str:
    """组装 netloc，剥离默认端口（http/ws=80，https/wss=443）。"""
...
def _looks_plausible_url(u: str) -> bool:
    """丢掉 logger.info / a.x.info / 7.error.cn / 单字母标签等假 host。"""
    host = _host_of(u)
    ...
    if len(labels) < 2 or any(len(lab) == 0 for lab in labels):
        return False
    if any(len(lab) == 1 and not lab.isdigit() for lab in labels[:-1]):
        return False
    # 数字开头短标签 + 普通词：7.error.cn / 1.logger.info
    if labels[0].isdigit() and len(labels[0]) <= 3:
        return False
    ...
    if labels[-2].lower() in ("error", "logger", "debug", "verbose") and labels[-1] in (
        ...
        return False
    if len(labels) >= 2 and all(len(lab) <= 2 for lab in labels[:-1]):
        return False
    return True
...
def is_noise_url(u: str) -> bool:
    ...
    if _IP_RE.match(host):
        if host.startswith(("127.", "0.")):
            return True
        if host in _PUBLIC_DNS_IPS or host in _EMULATOR_IPS:
            return True
...
def _scan_raw_for_urls(raw: bytes) -> set[str]:
    """从任意二进制里抠 URL（ASCII / UTF-16LE / 裸 IP:port）。"""
    found: set[str] = set()
    if not raw:
        return found
...
def _iter_urls(s: str):
    ...
    按优先级「完整 URL -> IP -> www -> 带路径裸域名 -> 裸 host」逐级提取并掩码，
...
def _normalize_base(value: str) -> list[str]:
    ...
    if "." not in host and not _IP_RE.match(host):
        return []
    out = []
